...
control	'V-233584' do
	title	"PostgreSQL must use #{input('org_name')[:acronym]}-approved cryptography to protect classified information in accordance with the 
...
#{input('org_name')[:acronym]}-approved cryptography for classified networks is hardware based. This requirement addresses the compatibility of 
...
If PostgreSQL is not using #{input('org_name')[:acronym]}-approved cryptography to protect classified information in accordance with applicable 
...
Consult network administration staff to determine whether the server is protected by #{input('org_name')[:acronym]}-approved encrypting devices. 
...
Deploy #{input('org_name')[:acronym]}-approved encrypting devices to protect the server on the network."
	impact 0.5
	tag severity: 'medium'
